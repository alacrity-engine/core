...
type AVLUnrestrictedSet[TKey Comparable] struct {
	tree *UnrestrictedAVLTree[TKey, TKey]
}

func (avl *AVLUnrestrictedSet[TKey]) SetPool(pool *mempool.Pool[*UnrestrictedAVLNode[TKey, TKey]]) {
	avl.tree.SetPool(pool)
}

func (set *AVLUnrestrictedSet[TKey]) Erase() error {
	return set.tree.Erase()
}

func (set *AVLUnrestrictedSet[TKey]) Add(key TKey) error {
	set.tree.Add(key, key)
	return nil
}

func (set *AVLUnrestrictedSet[TKey]) Remove(key TKey) error {
	set.tree.Remove(key)
	return nil
}

func (set *AVLUnrestrictedSet[TKey]) Search(key TKey) (bool, error) {
	return set.tree.Search(key) != nil, nil
}

func (set *AVLUnrestrictedSet[TKey]) VisitInOrder(visit func(key TKey)) {
	set.tree.VisitInOrder(func(node *UnrestrictedAVLNode[TKey, TKey]) {
		visit(node.Key())
	})
}
